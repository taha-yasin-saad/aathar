...
class Fortune extends Widget_Base {
	...
	protected function register_controls() {
		...
		$this->add_control(
			'direction',
			[ 
				'label'   => esc_html__( 'Direction', 'bdthemes-prime-slider' ) . BDTPS_PRO_NC,
				'type'    => Controls_Manager::SELECT,
				'default' => 'horizontal',
				'options' => [ 
					'horizontal' => esc_html__( 'Horizontal', 'bdthemes-prime-slider' ),
					'vertical'   => esc_html__( 'Vertical', 'bdthemes-prime-slider' ),
				],
			]
		);
		...
		$this->add_responsive_control(
			...
			[ 
				...
				'selectors' => [ 
					'{{WRAPPER}} .bdt-fortune-slider .horizontal .bdt-pagination' => 'bottom: {{SIZE}}px;',
					'{{WRAPPER}} .bdt-fortune-slider .vertical .bdt-pagination' => 'top: calc(100% - {{SIZE}}px);'
				],
			]
		);
	...
	public function render_footer() {
		...
		$this->add_render_attribute('nav', 'class', 'bdt-nav-pag-wrap reveal-muted ' . esc_attr($settings['direction']));
