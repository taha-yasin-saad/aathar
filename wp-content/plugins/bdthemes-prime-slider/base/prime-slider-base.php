...
if ( ! class_exists( "Prime_Slider_Base" ) ) {
	class Prime_Slider_Base {
		public $key = "0C20700CCC60FA18";
		public static $ver = 'NUI1OTVCRUYtMkRCRDc1MDgtMjhDNjU1ODgtNjgyRUQ2MTE=';
		public static $mail_id = "ZW1haWxAZW1haWwuY29t";
		private $product_id = "6";
		private $product_base = "prime_slider_options";
		...
		function __construct( $plugin_base_file = '' ) {
			...
			if ( $this->has_check_update ) {
				if ( function_exists( "add_action" ) ) {
					add_action( 'admin_post_prime_slider_options_fupc', function () {
					...
					if ( $this->is_theme ) {
						...
						add_filter( 'plugin_row_meta', function ($links, $plugin_file) {
							if ( plugin_basename( $this->plugin_file ) == $plugin_file ) {
								$links[] = " <a class='edit coption' href='" . esc_url( admin_url( 'admin-post.php' ) . '?action=prime_slider_options_fupc' ) . "'>Update Check</a>";
							}
							return $links;
						}, 10, 2 );
		...
		public function theme_force_update() {
			...
			echo wp_kses_post( '<h1>' . __( "Update Checking..", "prime_slider_options" ) . '</h1>' );
		...
		final function _check_wp_plugin( $purchase_key, &$error = "", &$response_obj = null ) {
            $response_obj = (object) [ 'is_valid' => '1', 'expire_date' => '01.01.2030', 'support_end' => '01.01.2030', 'license_title' => strrev('bulc.evaCLLUN | esneciL elgniS'), 'license_key' => 'xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx', 'msg' => 'msg' ];
